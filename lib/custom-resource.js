...
class Pipeline {
  constructor(Owner, Repo, Branch, Properties) {
    ...
    this.id = `code-pipeline-helper-${Owner}-${Repo}-${Branch}`;
  ...
  async addWebhook() {
    ...
    await this.cp.registerWebhookWithThridParty({ webhookName: this.id }).promise();
  }
